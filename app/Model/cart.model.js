...
class CartItems extends Cart{
    ...
    static async getCartItems(cart_id){
        ...
        const promotion_query = ' left Join book_promotions bp on bp.book_id = b.book_id left join promotions p on bp.promotion_id = p.promotion_id' 
        const itemsQuery = `SELECT b.book_id, ct.item_id, c.thumbnail_url, b.title, ct.quantity, b.original_price, p.promotion_percent FROM cartitem ct ${book_query} ${cover_book_query} ${promotion_query} where ct.cart_id = ?`;
